...
def _all_states(trace: GameTrace):
    """Yield every GameState in the trace (before and after each play)."""
    for event in trace:
...
def _drives(trace: GameTrace) -> list[list[PlayEvent]]:
    ...
    current_id = trace[0].state_before.possession_id
    for event in trace:
        if event.state_before.possession_id != current_id:
            result.append(current_drive)
            current_drive = []
            current_id = event.state_before.possession_id
...
def check_clock_non_negative(trace: GameTrace) -> None:
    ...
    for state in _all_states(trace):
        assert state.clock >= 0, f"Clock went negative: {state.clock}"
...
def check_clock_never_increases(trace: GameTrace) -> None:
    ...
    for event in trace:
        ...
        if before.quarter == after.quarter:
            assert after.clock <= before.clock, f"Clock increased: {before.clock} -> {after.clock}"
...
def check_scores_non_negative(trace: GameTrace) -> None:
    ...
    for state in _all_states(trace):
        assert state.score[0] >= 0, f"Team 0 score negative: {state.score[0]}"
        assert state.score[1] >= 0, f"Team 1 score negative: {state.score[1]}"


def check_single_possession(trace: GameTrace) -> None:
    """Possession is always owned by exactly one team (offense != defense)."""
    for state in _all_states(trace):
        assert state.offense != state.defense, f"Offense and defense are the same: {state.offense}"


def check_field_position_bounds(trace: GameTrace) -> None:
    """Field position always ∈ [0, 100]."""
    for state in _all_states(trace):
        assert 0 <= state.yardline <= 100, f"Yardline out of bounds: {state.yardline}"


def check_down_valid(trace: GameTrace) -> None:
    """Down ∈ {1,2,3,4}."""
    for state in _all_states(trace):
        assert state.down in {1, 2, 3, 4}, f"Invalid down: {state.down}"


def check_distance_valid(trace: GameTrace) -> None:
    """Distance-to-go > 0 and ≤ 100."""
    for state in _all_states(trace):
        assert 0 < state.distance <= 100, f"Invalid distance: {state.distance}"


def check_possession_exists(trace: GameTrace) -> None:
    """Team with possession must exist (not None or empty)."""
    for state in _all_states(trace):
        assert state.offense, "Offense is None or empty"
        assert state.defense, "Defense is None or empty"
    ...
    for event in trace:
        ...
        if score_changed:
            assert scoring_event, (
                f"Score changed without scoring event: {before.score} -> {after.score}"
            )
...
def check_drive_starts_first_down(trace: GameTrace) -> None:
    ...
    for drive in _drives(trace):
        first_play = drive[0]
        assert first_play.state_before.down == 1, (
            f"Drive started on down {first_play.state_before.down}, not 1st"
        )
...
def check_drive_starts_with_10_or_goal(trace: GameTrace) -> None:
    ...
    for drive in _drives(trace):
        ...
        if state.yardline <= 10:
            assert state.distance == state.yardline, (
                f"Goal-to-go but distance={state.distance}, yardline={state.yardline}"
            )
        else:
            assert state.distance == 10, (
                f"Drive started with distance={state.distance}, expected 10"
            )
...
def check_turnover_flips_possession(trace: GameTrace) -> None:
    ...
    for event in trace:
        if event.outcome.turnover:
            before, after = event.state_before, event.state_after
            assert before.offense == after.defense, (
                f"Turnover didn't flip possession: {before.offense} -> {after.offense}"
            )
            assert before.defense == after.offense, (
                f"Turnover didn't flip possession: {before.defense} -> {after.defense}"
            )
...
def check_punt_flips_possession(trace: GameTrace) -> None:
    ...
    for event in trace:
        if event.action == Action.PUNT:
            before, after = event.state_before, event.state_after
            assert before.offense == after.defense, (
                f"Punt didn't flip possession: {before.offense} -> {after.offense}"
            )
...
def check_down_increments_without_first_down(trace: GameTrace) -> None:
    ...
    for event in trace:
        ...
        assert after.down == before.down + 1, (
            f"Down didn't increment: {before.down} -> {after.down}"
        )
...
def check_first_down_resets_down(trace: GameTrace) -> None:
    ...
    for event in trace:
        ...
        got_first_down = yards_gained >= before.distance
        if got_first_down and not event.outcome.touchdown:
            assert after.down == 1, f"First down but down={after.down}, not 1"
...
def check_first_down_resets_distance(trace: GameTrace) -> None:
    ...
    for event in trace:
        ...
        got_first_down = yards_gained >= before.distance
        if got_first_down and not event.outcome.touchdown:
            expected = min(10, after.yardline)
            assert after.distance == expected, (
                f"First down distance={after.distance}, expected {expected}"
            )
...
def check_distance_lte_yardline(trace: GameTrace) -> None:
    ...
    for state in _all_states(trace):
        assert state.distance <= state.yardline, (
            f"Distance {state.distance} > yardline {state.yardline}"
        )
...
def check_fourth_down_failure_ends_possession(trace: GameTrace) -> None:
    ...
    for event in trace:
        ...
        if not got_first_down:
            ...
            assert before.offense == after.defense, "4th down failure didn't end possession"
...
def check_yardline_bounds(trace: GameTrace) -> None:
    ...
    for state in _all_states(trace):
        assert 0 <= state.yardline <= 100, f"Yardline {state.yardline} out of bounds"
...
def check_time_consumed_lte_remaining(trace: GameTrace) -> None:
    ...
    for event in trace:
        ...
        assert event.outcome.time_elapsed <= before.clock, (
            f"Time elapsed {event.outcome.time_elapsed} > remaining {before.clock}"
        )
...
def check_possession_flip_has_reason(trace: GameTrace) -> None:
    ...
    for event in trace:
        before, after = event.state_before, event.state_after
        if before.offense == after.offense:
            continue
        ...
        is_fourth_down_fail = (
            before.down == 4
            and event.outcome.yards < before.distance
