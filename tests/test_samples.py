...
def test_filters_by_down(mock_play_data: pl.DataFrame):
    ...
    indices = nfl_sim_core.filter_window(offense_matrix, down=1, dist=10, yardline=75, wp=0.5)
...
def test_filters_by_yardline_window(mock_play_data: pl.DataFrame):
    offense_df, offense_matrix, _, _ = build_sample_pairs(mock_play_data, "KC")
    indices = nfl_sim_core.filter_window(offense_matrix, down=1, dist=10, yardline=70, wp=0.5)
    filtered = offense_df[indices.tolist()]
...
def test_goal_to_go_adjusts_distance(mock_play_data: pl.DataFrame):
    ...
    indices = nfl_sim_core.filter_window(offense_matrix, down=1, dist=10, yardline=5, wp=0.5)
    filtered = offense_df[indices.tolist()]
...
def test_raises_when_no_matches():
    ...
    with pytest.raises(AssertionError):
        fetch_like_play(offense_df, offense_matrix, down=4, dist=10, yardline=75, wp=0.5)
...
def test_returns_filtered_sample(mock_play_data: pl.DataFrame):
    offense_df, offense_matrix, _, _ = build_sample_pairs(mock_play_data, "KC")
    result = fetch_like_play(offense_df, offense_matrix, down=1, dist=10, yardline=70, wp=0.5)
    assert len(result) == 1
